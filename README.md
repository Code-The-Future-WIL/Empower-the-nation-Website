<h1>🌍 Empowering the Nation Website</h1>

<img width="158" height="158" alt="Empowering the Nation Logo" src="https://github.com/user-attachments/assets/fb9ca0bc-9e8b-4ff8-a69f-bdd99770ba45" />

<h2>🎥 YouTube Demo</h2>
<p>
  <a href= "https://youtu.be/a19rvYSnpzc?feature=shared" target="_blank">
    Watch the video here
  </a>
</p>

<h2>🌟 Project Overview</h2>
<p>
  <strong>Empowering the Nation</strong> is a community-focused website that provides practical
  skills training for <strong>domestic workers</strong> and <strong>gardeners</strong>. Founded by
  <strong>Precious Radebe</strong>, the platform offers both short- and long-term courses aimed at
  improving employability, entrepreneurship, and community upliftment.
</p>
<p>
  The website enables users to explore courses, calculate fees, register online, and easily contact
  the organization.
</p>

<h2>💡 Core Features</h2>
<ul>
  <li>Course Catalog & Information</li>
  <li>Online Fee Calculator</li>
  <li>Consultation & Registration Form</li>
  <li>Responsive User-Friendly Navigation</li>
</ul>

<h2>🖥️ Technical Features</h2>
<ul>
  <li>Responsive Web Design (HTML, CSS, JavaScript)</li>
  <li>Interactive Navigation & Form Handling</li>
  <li>Embedded Google Maps for Training Locations</li>
  <li>Clean, Accessible User Interface</li>
</ul>

<h2>💼 Business Features</h2>
<ul>
  <li>Course Management – Display and update course offerings</li>
  <li>Contact Management – Handle user inquiries and registration forms</li>
</ul>

<h2>🔄 Workflow & User Journey</h2>
<ol>
  <li>Homepage (Welcome Screen)</li>
  <li>Six-Month and Six-Week Courses</li>
  <li>Fee Calculator</li>
  <li>Registration / Contact Page</li>
  <li>Location Page (with Integrated Google Maps)</li>
</ol>

<h2>📱 Website Pages</h2>
<ol>
  <li>Home (Welcome)</li>
  <li>Six-Month Courses</li>
  <li>Six-Week Courses</li>
  <li>First Aid</li>
  <li>Sewing</li>
  <li>Landscaping</li>
  <li>Life Skills</li>
  <li>Child Minding</li>
  <li>Cooking</li>
  <li>Garden Maintenance</li>
  <li>Fee Calculator</li>
  <li>Contact / Registration</li>
</ol>

<h2>📚 References</h2>
<ul>
  <li>
    Android Developers. (2025). <em>Intents and intent filters</em> [Documentation].
    <a href="https://developer.android.com/guide/components/intents-filters" target="_blank">
      https://developer.android.com/guide/components/intents-filters
    </a>
    [Accessed: 5 October 2025].
  </li>
  <li>
    Canva. (2025). <em>Back Button Logo</em> [Graphic design].
    <a href="https://www.canva.com" target="_blank">www.canva.com</a> [Accessed: 6 October 2025].
  </li>
  <li>
    Canva. (2025). <em>Contact Button Logo</em> [Graphic design].
    <a href="https://www.canva.com" target="_blank">www.canva.com</a> [Accessed: 6 October 2025].
  </li>
  <li>
    Canva. (2025). <em>Logo Design created in Canva</em>.
    <a href="https://www.canva.com/design/DAGvULoGawo/x_8eaGFyIBNAzwXsFNInDQ/edit?ui=eyJIIjp7IkEiOnRydWV9fQ" target="_blank">
      View Logo Design
    </a> [Accessed: 07 August 2025].
  </li>
  <li>
    Google Maps. (2025). <em>Map Locations</em> [Digital Maps].
    <ul>
      <li>
        <a href="https://www.google.com/maps/dir/The+IIE's+Varsity+College+-+Durban+North,+Radar+Drive,+Durban+North,+Durban/463+Main+Street,+Jeppestown,+Johannesburg" target="_blank">Map Location 1</a>
      </li>
      <li>
        <a href="https://www.google.com/maps/dir/The+IIE's+Varsity+College+-+Durban+North,+Radar+Drive,+Durban+North,+Durban/33+Rose+Avenue,+Lenasia" target="_blank">Map Location 2</a>
      </li>
      <li>
        <a href="https://www.google.com/maps/dir/The+IIE's+Varsity+College+-+Durban+North,+Radar+Drive,+Durban+North,+Durban/24+Market+Road,+Newlands,+Johannesburg" target="_blank">Map Location 3</a>
      </li>
    </ul>
 
